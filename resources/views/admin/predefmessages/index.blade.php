    ...
    <div class="mx-auto">
        <div class="bg-white rounded-xl shadow-lg border border-gray-100">
            <div class="p-4 sm:p-6 lg:p-8">
                <form id="filterForm" action="{{ route('admin.predefined-messages.index') }}" method="GET">
                    ...
                    <div class="flex flex-col lg:flex-row justify-between items-center mt-6 gap-4">
                        <div class="flex flex-wrap items-center gap-2 space-x-2">
                            ...
                            <a href="{{ route('admin.predefined-messages.export', ['type' => 'excel'] + request()->all()) }}"
                                class="inline-flex items-center px-4 py-2 bg-white text-blue-800 rounded-lg text-sm font-medium shadow-md">
                                Excel
                            </a>
                            <a href="{{ route('admin.predefined-messages.export', ['type' => 'csv'] + request()->all()) }}"
                                class="inline-flex items-center px-4 py-2 bg-white text-blue-800 rounded-lg text-sm font-medium shadow-md">
                                CSV
                            </a>
                            <a href="{{ route('admin.predefined-messages.export', ['type' => 'pdf'] + request()->all()) }}"
                                class="inline-flex items-center px-4 py-2 bg-white text-blue-800 rounded-lg text-sm font-medium shadow-md">
                                PDF
                            </a>
                        </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 relative">
                                <thead class="bg-blue-50">
                                    <tr>
                                        ...
                                        <th scope="col"
                                            class="sticky top-0 bg-blue-50 px-4 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider border-b border-gray-200">
                                            {!! sortLink('message_name', 'Message Name') !!}
                                        </th>
                                        <th scope="col"
                                            class="sticky top-0 bg-blue-50 px-4 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider border-b border-gray-200">
                                            {!! sortLink('message_remarks', 'Remarks') !!}
                                        </th>
                                        <th scope="col"
                                            class="sticky top-0 bg-blue-50 px-4 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider border-b border-gray-200">
                                ...
                                <tbody class="bg-white">
                                    ...
                                    @forelse ($preDefinedMessages as $message)
                                        <tr
                                            ...
                                            <td class="px-4 py-4 whitespace-normal text-sm text-gray-700 max-w-md break-words">
                                                {{ Str::limit($message->message_remarks, 150) }} {{-- Show limited remarks --}}
                                            </td>
                                            <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-700">
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap text-sm">
                                                <div class="flex items-center space-x-3">
                                                    {{-- Update route names --}}
                                                    <a href="{{ route('admin.predefined-messages.show', ['predefined_message' => $message->id]) }}"
                                                    ...
                                                    <a href="{{ route('admin.predefined-messages.edit', ['predefined_message' => $message->id]) }}"
                                                    ...
                                                    {{-- Update route name and form class --}}
                                                    <form action="{{ route('admin.predefined-messages.destroy', ['predefined_message' => $message->id]) }}"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="px-6 py-10 text-center"> {{-- Updated colspan --}}
                ...
                <div class="mt-6 px-4 sm:px-0 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div class="flex items-center gap-4">
                        {{-- Update variable name --}}
                        @if ($preDefinedMessages->total() > 0)
                            <p class="text-sm text-gray-500">
                                Showing
                                <span class="font-medium">{{ $preDefinedMessages->firstItem() }}</span>
                                to
                                <span class="font-medium">{{ $preDefinedMessages->lastItem() }}</span>
                                of
                                <span class="font-medium">{{ $preDefinedMessages->total() }}</span>
                                results
                            </p>
                        @endif
                        @php $options = [10, 25, 50, 100]; @endphp
                        {{-- Update variable name --}}
                        @if ($preDefinedMessages->total() > ($options[0] ?? 10))
                    ...
                    <div>
                        {{-- Update variable name --}}
                        @if ($preDefinedMessages->hasPages())
                            {{ $preDefinedMessages->appends(request()->query())->links() }}
                        @endif
                    </div>
                </div>
            </div>
