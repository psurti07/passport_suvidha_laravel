...
<div class="mx-auto">
    <div class="bg-white rounded-xl shadow-lg border border-gray-100">
        <div class="p-4 sm:p-6 lg:p-8">
            <form id="filterForm" action="{{ route('admin.customers.today') }}" method="GET">
                {{-- Main Header with Title and Counts --}}
                <div class="flex flex-col sm:flex-row justify-between space-y-2 sm:space-y-0 mb-6">
                    ...
                    {{-- Restyled Inline Counts --}}
                    <div class="flex flex-wrap items-center gap-x-3 gap-y-2">
                        <span class="inline-flex items-center px-3 py-1 rounded-md shadow-sm font-medium bg-blue-100 text-blue-800 text-base">
                            Total:
                            <span class="font-bold ml-2 text-lg">{{ $totalTodayCount }}</span>
                        </span>
                        <span class="inline-flex items-center px-3 py-1 rounded-md shadow-sm font-medium bg-green-100 text-green-800 text-base">
                            Paid:
                            <span class="font-bold ml-2 text-lg">{{ $paidTodayCount }}</span>
                        </span>
                        <span class="inline-flex items-center px-3 py-1 rounded-md shadow-sm font-medium bg-yellow-100 text-yellow-800 text-base">
                            Leads:
                            <span class="font-bold ml-2 text-lg">{{ $leadTodayCount }}</span>
                        </span>
                    </div>
                    {{-- Hidden inputs can remain associated with the form but outside the main header flex --}}
                </div>
                ...
                <div class="flex flex-col lg:flex-row justify-between items-center mt-6 gap-4">
                     <div class="flex flex-wrap items-center gap-2 space-x-2">
                         ...
                         <button type="button" onclick="copyToClipboard()"
                            ...
                            {{-- Update route names for export --}}
                            <a href="{{ route('admin.customers.export', ['type' => 'excel', 'source' => 'today'] + request()->except(['from_date', 'to_date'])) }}"
                                class="inline-flex items-center px-4 py-2 bg-white text-blue-800 rounded-lg text-sm font-medium shadow-md hover:bg-gray-100 transition-colors duration-150">
                                Excel
                            </a>
                            <a href="{{ route('admin.customers.export', ['type' => 'csv', 'source' => 'today'] + request()->except(['from_date', 'to_date'])) }}"
                                class="inline-flex items-center px-4 py-2 bg-white text-blue-800 rounded-lg text-sm font-medium shadow-md hover:bg-gray-100 transition-colors duration-150">
                                CSV
                            </a>
                            <a href="{{ route('admin.customers.export', ['type' => 'pdf', 'source' => 'today'] + request()->except(['from_date', 'to_date'])) }}"
                                class="inline-flex items-center px-4 py-2 bg-white text-blue-800 rounded-lg text-sm font-medium shadow-md hover:bg-gray-100 transition-colors duration-150">
                                PDF
                            </a>
                        </div>
                             ...
                             <tbody class="bg-white">
                                @forelse($customers as $customer)
                                 <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100 transition-colors duration-150 cursor-pointer"
                                     onclick="window.location.href='{{ route('admin.customers.show', $customer) }}?previous_url={{ urlencode(request()->fullUrl()) }}'">
            ...
            <div class="mt-6 px-4 sm:px-0 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div class="flex items-center gap-4">
                    ...
                    @if ($customers->total() > ($options[0] ?? 10))
                        <div class="flex items-center gap-2">
...
<script>
    ...
    function copyToClipboard() {
        ...
        // Check if the only row is the "No customers found" row (using colspan="6")
    ...
    @php
        ...
        function sortLink($column, $label)
        {
            ...
            $route = 'admin.customers.today'; // Use the new route name
            $queryParams = array_merge(request()->except(['page', 'from_date', 'to_date']), [ // Exclude date params
                'sort_by' => $column,
