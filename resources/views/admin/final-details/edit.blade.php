...
@section('title', 'Edit Final Detail')
...
<div class="mx-auto">
    <div class="bg-white rounded-xl shadow-lg border border-gray-100">
        <div class="p-4 sm:p-6 lg:p-8">
            ...
            <form action="{{ route('admin.final-details.update', $finalDetail) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    {{-- Customer --}}
                    <div>
                        ...
                        <select id="customer_id" name="customer_id" required
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ (old('customer_id', $finalDetail->customer_id) == $customer->id) ? 'selected' : '' }}>
                        ...
                        @if($finalDetail->file_path)
                            <div class="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-100">
                                <div class="flex items-center">
                                    ...
                                    <a href="{{ Storage::url($finalDetail->file_path) }}" target="_blank" class="text-blue-600 hover:underline ml-2">
                                        <span class="text-sm">{{ basename($finalDetail->file_path) }}</span>
                                    </a>
                                </div>
                                <p class="mt-2 text-xs text-gray-500">Uploaded on: {{ $finalDetail->upload_date ? $finalDetail->upload_date->format('d M Y, h:i A') : 'N/A' }}</p>
                            </div>
                        @endif
                    ...
                    <div>
                        <div class="flex items-center">
                            <input id="is_approved" name="is_approved" type="checkbox" value="1" {{ $finalDetail->is_approved ? 'checked' : '' }}
                        ...
                        @if($finalDetail->is_approved)
                            <div class="mt-2 text-xs text-gray-600">
                                ...
                                <p>Approved date: {{ $finalDetail->approved_date ? $finalDetail->approved_date->format('d M Y, h:i A') : 'N/A' }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="bg-blue-50 p-4 rounded-lg border border-blue-100">
                        ...
                        <p class="text-xs text-gray-600">
                            ...
                            Upload date: {{ $finalDetail->upload_date ? $finalDetail->upload_date->format('d M Y, h:i A') : 'N/A' }}
                        </p>
