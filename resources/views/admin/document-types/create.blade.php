    ...
    <div class="mx-auto">
        <div class="bg-white rounded-xl shadow-lg border border-gray-100">
            <div class="p-4 sm:p-6 lg:p-8">
                ...
                <form action="{{ route('admin.document-types.store') }}" method="POST" novalidate>
                    @csrf
                    <div class="space-y-6">
                        {{-- Name --}}
                        <div>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" placeholder="Enter document type description"
                                class="w-full border border-gray-300 rounded-lg text-sm px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
