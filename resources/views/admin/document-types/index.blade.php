    ...
    <div class="mx-auto">
        <div class="bg-white rounded-xl shadow-lg border border-gray-100">
            <div class="p-4 sm:p-6 lg:p-8">
                <form id="filterForm" action="{{ route('admin.document-types.index') }}" method="GET">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 relative">
                                <thead class="bg-blue-50">
                                    <tr>
                                        ...
                                        <th scope="col"
                                            class="sticky top-0 bg-blue-50 px-4 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider border-b border-gray-200">
                                            {!! sortLink('description', 'Description') !!}
                                        </th>
                                        <th scope="col"
                                            class="sticky top-0 bg-blue-50 px-4 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider border-b border-gray-200">
                                            {!! sortLink('is_mandatory', 'Mandatory') !!}
                                        </th>
                                        <th scope="col"
                                            class="sticky top-0 bg-blue-50 px-4 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider border-b border-gray-200">
                                    ...
                                    @forelse ($documentTypes as $documentType)
                                        <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100 transition-colors duration-150">
                                            ...
                                            <td class="px-4 py-4 whitespace-normal text-sm text-gray-700 max-w-md break-words">
                                                {{ Str::limit($documentType->description, 150) }}
                                            </td>
                                            <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-700">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $documentType->is_mandatory ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                                    {{ $documentType->is_mandatory ? 'Yes' : 'No' }}
                                                </span>
                                            </td>
                                            <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-700">
                                                {{ $documentType->created_at->format('d/m/Y H:i:s') }}
                                            </td>
                    ...
                    <div class="flex items-center gap-4">
                        @if ($documentTypes->total() > 0)
                            <p class="text-sm text-gray-500">
                                Showing
                                <span class="font-medium">{{ $documentTypes->firstItem() }}</span>
                                to
                                <span class="font-medium">{{ $documentTypes->lastItem() }}</span>
                                of
                                <span class="font-medium">{{ $documentTypes->total() }}</span>
                                results
                            </p>
                        @endif
                        @php $options = [10, 25, 50, 100]; @endphp
                        @if ($documentTypes->total() > ($options[0] ?? 10))
