...
class StarOfferController extends Controller
{
    ...
    {
        return view('admin.star-offers.index');
    }
    ...
    public function customer(OfferOrder $starOffer)
    {
        $starOffer->update([
            'is_customer' => 1,
        ]);

        return redirect()->route('admin.star-offers.index')
            ->with('success', 'Lead converted to customer successfully.');
    }

    public function lead(OfferOrder $starOffer)
    {
        $starOffer->update([
            'is_customer' => 0,
        ]);
